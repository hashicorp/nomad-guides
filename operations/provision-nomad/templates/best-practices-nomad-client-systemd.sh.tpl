...
echo "[---Begin best-practices-nomad-client-systemd.sh---]"
...
sudo mkdir -pm 0755 $CONSUL_TLS_DIR $NOMAD_TLS_DIR $WETTY_TLS_DIR

echo "Write certs to TLS directories"
cat <<EOF | sudo tee $CONSUL_TLS_DIR/consul-ca.crt $NOMAD_TLS_DIR/consul-ca.crt $NOMAD_TLS_DIR/vault-ca.crt $NOMAD_TLS_DIR/nomad-ca.crt $WETTY_TLS_DIR/wetty-ca.crt
${ca_crt}
EOF
cat <<EOF | sudo tee $CONSUL_TLS_DIR/consul.crt $NOMAD_TLS_DIR/consul.crt $NOMAD_TLS_DIR/vault.crt $NOMAD_TLS_DIR/nomad.crt $WETTY_TLS_DIR/wetty.crt
${leaf_crt}
EOF
cat <<EOF | sudo tee $CONSUL_TLS_DIR/consul.key $NOMAD_TLS_DIR/consul.key $NOMAD_TLS_DIR/vault.key $NOMAD_TLS_DIR/nomad.key $WETTY_TLS_DIR/wetty.key
${leaf_key}
EOF
...
echo "[---best-practices-nomad-client-systemd.sh Complete---]"
